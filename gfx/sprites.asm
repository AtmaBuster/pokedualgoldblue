INCLUDE "constants.asm"

floating_section
IF DEF(_GOLD)
PlayerSpriteGFX::
ENDC
ChrisSpriteGFX::               INCBIN "gfx/sprites/chris.2bpp"
floating_section
IF DEF(_GOLD)
PlayerBikeSpriteGFX::
ENDC
ChrisBikeSpriteGFX::           INCBIN "gfx/sprites/chris_bike.2bpp"
floating_section
GameboyKidSpriteGFX::          INCBIN "gfx/sprites/gameboy_kid.2bpp"
floating_section
SilverSpriteGFX::              INCBIN "gfx/sprites/silver.2bpp"
floating_section
OakSpriteGFX::                 INCBIN "gfx/sprites/oak.2bpp"
floating_section
IF DEF(_BLUE)
PlayerSpriteGFX::
ENDC
RedSpriteGFX::                 INCBIN "gfx/sprites/red.2bpp"
floating_section
BlueSpriteGFX::                INCBIN "gfx/sprites/blue.2bpp"
floating_section
BillSpriteGFX::                INCBIN "gfx/sprites/bill.2bpp"
floating_section
ElderSpriteGFX::               INCBIN "gfx/sprites/elder.2bpp"
floating_section
JanineSpriteGFX::              INCBIN "gfx/sprites/janine.2bpp"
floating_section
KurtSpriteGFX::                INCBIN "gfx/sprites/kurt.2bpp"
floating_section
MomSpriteGFX::                 INCBIN "gfx/sprites/mom.2bpp"
floating_section
BlaineSpriteGFX::              INCBIN "gfx/sprites/blaine.2bpp"
floating_section
RedsMomSpriteGFX::             INCBIN "gfx/sprites/reds_mom.2bpp"
floating_section
DaisySpriteGFX::               INCBIN "gfx/sprites/daisy.2bpp"
floating_section
ElmSpriteGFX::                 INCBIN "gfx/sprites/elm.2bpp"
floating_section
WillSpriteGFX::                INCBIN "gfx/sprites/will.2bpp"
floating_section
FalknerSpriteGFX::             INCBIN "gfx/sprites/falkner.2bpp"
floating_section
WhitneySpriteGFX::             INCBIN "gfx/sprites/whitney.2bpp"
floating_section
BugsySpriteGFX::               INCBIN "gfx/sprites/bugsy.2bpp"
floating_section
MortySpriteGFX::               INCBIN "gfx/sprites/morty.2bpp"
floating_section
ChuckSpriteGFX::               INCBIN "gfx/sprites/chuck.2bpp"
floating_section
JasmineSpriteGFX::             INCBIN "gfx/sprites/jasmine.2bpp"
floating_section
PryceSpriteGFX::               INCBIN "gfx/sprites/pryce.2bpp"
floating_section
ClairSpriteGFX::               INCBIN "gfx/sprites/clair.2bpp"
floating_section
BrockSpriteGFX::               INCBIN "gfx/sprites/brock.2bpp"
floating_section
KarenSpriteGFX::               INCBIN "gfx/sprites/karen.2bpp"
floating_section
BrunoSpriteGFX::               INCBIN "gfx/sprites/bruno.2bpp"
floating_section
MistySpriteGFX::               INCBIN "gfx/sprites/misty.2bpp"
floating_section
LanceSpriteGFX::               INCBIN "gfx/sprites/lance.2bpp"
floating_section
SurgeSpriteGFX::               INCBIN "gfx/sprites/surge.2bpp"
floating_section
ErikaSpriteGFX::               INCBIN "gfx/sprites/erika.2bpp"
floating_section
KogaSpriteGFX::                INCBIN "gfx/sprites/koga.2bpp"
floating_section
SabrinaSpriteGFX::             INCBIN "gfx/sprites/sabrina.2bpp"
floating_section
CooltrainerMSpriteGFX::        INCBIN "gfx/sprites/cooltrainer_m.2bpp"
floating_section
CooltrainerFSpriteGFX::        INCBIN "gfx/sprites/cooltrainer_f.2bpp"
floating_section
BugCatcherSpriteGFX::          INCBIN "gfx/sprites/bug_catcher.2bpp"
floating_section
TwinSpriteGFX::                INCBIN "gfx/sprites/twin.2bpp"
floating_section
YoungsterSpriteGFX::           INCBIN "gfx/sprites/youngster.2bpp"
floating_section
LassSpriteGFX::                INCBIN "gfx/sprites/lass.2bpp"
floating_section
TeacherSpriteGFX::             INCBIN "gfx/sprites/teacher.2bpp"
floating_section
BeautySpriteGFX::              INCBIN "gfx/sprites/beauty.2bpp"
floating_section
SuperNerdSpriteGFX::           INCBIN "gfx/sprites/super_nerd.2bpp"
floating_section
RockerSpriteGFX::              INCBIN "gfx/sprites/rocker.2bpp"
floating_section
PokefanMSpriteGFX::            INCBIN "gfx/sprites/pokefan_m.2bpp"
floating_section
PokefanFSpriteGFX::            INCBIN "gfx/sprites/pokefan_f.2bpp"
floating_section
GrampsSpriteGFX::              INCBIN "gfx/sprites/gramps.2bpp"
floating_section
GrannySpriteGFX::              INCBIN "gfx/sprites/granny.2bpp"
floating_section
SwimmerGuySpriteGFX::          INCBIN "gfx/sprites/swimmer_guy.2bpp"
floating_section
SwimmerGirlSpriteGFX::         INCBIN "gfx/sprites/swimmer_girl.2bpp"
floating_section
BigSnorlaxSpriteGFX::          INCBIN "gfx/sprites/big_snorlax.2bpp"
floating_section
SurfingPikachuSpriteGFX::      INCBIN "gfx/sprites/surfing_pikachu.2bpp"
floating_section
RocketSpriteGFX::              INCBIN "gfx/sprites/rocket.2bpp"
floating_section
RocketGirlSpriteGFX::          INCBIN "gfx/sprites/rocket_girl.2bpp"
floating_section
NurseSpriteGFX::               INCBIN "gfx/sprites/nurse.2bpp"
floating_section
LinkReceptionistSpriteGFX::    INCBIN "gfx/sprites/link_receptionist.2bpp"
floating_section
ClerkSpriteGFX::               INCBIN "gfx/sprites/clerk.2bpp"
floating_section
FisherSpriteGFX::              INCBIN "gfx/sprites/fisher.2bpp"
floating_section
FishingGuruSpriteGFX::         INCBIN "gfx/sprites/fishing_guru.2bpp"
floating_section
ScientistSpriteGFX::           INCBIN "gfx/sprites/scientist.2bpp"
floating_section
KimonoGirlSpriteGFX::          INCBIN "gfx/sprites/kimono_girl.2bpp"
floating_section
SageSpriteGFX::                INCBIN "gfx/sprites/sage.2bpp"
floating_section
UnusedGuySpriteGFX::           INCBIN "gfx/sprites/unused_guy.2bpp"
floating_section
GentlemanSpriteGFX::           INCBIN "gfx/sprites/gentleman.2bpp"
floating_section
BlackBeltSpriteGFX::           INCBIN "gfx/sprites/black_belt.2bpp"
floating_section
ReceptionistSpriteGFX::        INCBIN "gfx/sprites/receptionist.2bpp"
floating_section
OfficerSpriteGFX::             INCBIN "gfx/sprites/officer.2bpp"
floating_section
CalSpriteGFX::                 INCBIN "gfx/sprites/cal.2bpp"
floating_section
SlowpokeSpriteGFX::            INCBIN "gfx/sprites/slowpoke.2bpp"
floating_section
CaptainSpriteGFX::             INCBIN "gfx/sprites/captain.2bpp"
floating_section
BigLaprasSpriteGFX::           INCBIN "gfx/sprites/big_lapras.2bpp"
floating_section
GymGuideSpriteGFX::            INCBIN "gfx/sprites/gym_guide.2bpp"
floating_section
SailorSpriteGFX::              INCBIN "gfx/sprites/sailor.2bpp"
floating_section
BikerSpriteGFX::               INCBIN "gfx/sprites/biker.2bpp"
floating_section
PharmacistSpriteGFX::          INCBIN "gfx/sprites/pharmacist.2bpp"
floating_section
MonsterSpriteGFX::             INCBIN "gfx/sprites/monster.2bpp"
floating_section
FairySpriteGFX::               INCBIN "gfx/sprites/fairy.2bpp"
floating_section
BirdSpriteGFX::                INCBIN "gfx/sprites/bird.2bpp"
floating_section
DragonSpriteGFX::              INCBIN "gfx/sprites/dragon.2bpp"
floating_section
BigOnixSpriteGFX::             INCBIN "gfx/sprites/big_onix.2bpp"
floating_section
N64SpriteGFX::                 INCBIN "gfx/sprites/n64.2bpp"
floating_section
SudowoodoSpriteGFX::           INCBIN "gfx/sprites/sudowoodo.2bpp"
floating_section
SurfSpriteGFX::                INCBIN "gfx/sprites/surf.2bpp"
floating_section
PokeBallSpriteGFX::            INCBIN "gfx/sprites/poke_ball.2bpp"
floating_section
PokedexSpriteGFX::             INCBIN "gfx/sprites/pokedex.2bpp"
floating_section
PaperSpriteGFX::               INCBIN "gfx/sprites/paper.2bpp"
floating_section
VirtualBoySpriteGFX::          INCBIN "gfx/sprites/virtual_boy.2bpp"
floating_section
OldLinkReceptionistSpriteGFX:: INCBIN "gfx/sprites/old_link_receptionist.2bpp"
floating_section
RockSpriteGFX::                INCBIN "gfx/sprites/rock.2bpp"
floating_section
BoulderSpriteGFX::             INCBIN "gfx/sprites/boulder.2bpp"
floating_section
SnesSpriteGFX::                INCBIN "gfx/sprites/snes.2bpp"
floating_section
FamicomSpriteGFX::             INCBIN "gfx/sprites/famicom.2bpp"
floating_section
FruitTreeSpriteGFX::           INCBIN "gfx/sprites/fruit_tree.2bpp"
floating_section
GoldTrophySpriteGFX::          INCBIN "gfx/sprites/gold_trophy.2bpp"
floating_section
SilverTrophySpriteGFX::        INCBIN "gfx/sprites/silver_trophy.2bpp"
floating_section
KantoBlueSpriteGFX::           INCBIN "gfx/sprites/kanto_blue.2bpp"
floating_section
KantoYoungsterSpriteGFX::      INCBIN "gfx/sprites/kanto_youngster.2bpp"
floating_section
LittleGirlSpriteGFX::          INCBIN "gfx/sprites/little_girl.2bpp"
floating_section
KantoBirdSpriteGFX::           INCBIN "gfx/sprites/kanto_bird.2bpp"
floating_section
GamblerSpriteGFX::             INCBIN "gfx/sprites/gambler.2bpp"
floating_section
KantoSuperNerdSpriteGFX::      INCBIN "gfx/sprites/kanto_super_nerd.2bpp"
floating_section
GirlSpriteGFX::                INCBIN "gfx/sprites/girl.2bpp"
floating_section
HikerSpriteGFX::               INCBIN "gfx/sprites/hiker.2bpp"
floating_section
KantoBeautySpriteGFX::         INCBIN "gfx/sprites/kanto_beauty.2bpp"
floating_section
KantoSailorSpriteGFX::         INCBIN "gfx/sprites/kanto_sailor.2bpp"
floating_section
CookSpriteGFX::                INCBIN "gfx/sprites/cook.2bpp"
floating_section
BikeShopClerkSpriteGFX::       INCBIN "gfx/sprites/bike_shop_clerk.2bpp"
floating_section
GiovanniSpriteGFX::            INCBIN "gfx/sprites/giovanni.2bpp"
floating_section
KantoRocketSpriteGFX::         INCBIN "gfx/sprites/kanto_rocket.2bpp"
floating_section
ChannelerSpriteGFX::           INCBIN "gfx/sprites/channeler.2bpp"
floating_section
WaiterSpriteGFX::              INCBIN "gfx/sprites/waiter.2bpp"
floating_section
KantoErikaSpriteGFX::          INCBIN "gfx/sprites/kanto_erika.2bpp"
floating_section
BrunetteGirlSpriteGFX::        INCBIN "gfx/sprites/brunette_girl.2bpp"
floating_section
KantoScientistSpriteGFX::      INCBIN "gfx/sprites/kanto_scientist.2bpp"
floating_section
KantoRockerSpriteGFX::         INCBIN "gfx/sprites/kanto_rocker.2bpp"
floating_section
SafariZoneWorkerSpriteGFX::    INCBIN "gfx/sprites/safari_zone_worker.2bpp"
floating_section
KantoGymGuideSpriteGFX::       INCBIN "gfx/sprites/kanto_gym_guide.2bpp"
floating_section
KantoGrampsSpriteGFX::         INCBIN "gfx/sprites/kanto_gramps.2bpp"
floating_section
KantoClerkSpriteGFX::          INCBIN "gfx/sprites/kanto_clerk.2bpp"
floating_section
KantoGrannySpriteGFX::         INCBIN "gfx/sprites/kanto_granny.2bpp"
floating_section
KantoNurseSpriteGFX::          INCBIN "gfx/sprites/kanto_nurse.2bpp"
floating_section
SilphPresidentSpriteGFX::      INCBIN "gfx/sprites/silph_president.2bpp"
floating_section
SilphWorkerSpriteGFX::         INCBIN "gfx/sprites/silph_worker.2bpp"
floating_section
WardenSpriteGFX::              INCBIN "gfx/sprites/warden.2bpp"
floating_section
KantoKogaSpriteGFX::           INCBIN "gfx/sprites/kanto_koga.2bpp"
floating_section
GuardSpriteGFX::               INCBIN "gfx/sprites/guard.2bpp"
floating_section
KantoMomSpriteGFX::            INCBIN "gfx/sprites/kanto_mom.2bpp"
floating_section
BaldingGuySpriteGFX::          INCBIN "gfx/sprites/balding_guy.2bpp"
floating_section
LittleBoySpriteGFX::           INCBIN "gfx/sprites/little_boy.2bpp"
floating_section
AgathaSpriteGFX::              INCBIN "gfx/sprites/agatha.2bpp"
floating_section
LoreleiSpriteGFX::             INCBIN "gfx/sprites/lorelei.2bpp"
floating_section
SeelSpriteGFX::                INCBIN "gfx/sprites/seel.2bpp"
floating_section
FossilSpriteGFX::              INCBIN "gfx/sprites/fossil.2bpp"
floating_section
KantoBoulderSpriteGFX::        INCBIN "gfx/sprites/kanto_boulder.2bpp"
floating_section
ClipboardSpriteGFX::           INCBIN "gfx/sprites/clipboard.2bpp"
floating_section
KantoSnorlaxSpriteGFX::        INCBIN "gfx/sprites/kanto_snorlax.2bpp"
floating_section
OldAmberSpriteGFX::            INCBIN "gfx/sprites/old_amber.2bpp"
floating_section
GamblerAsleepSpriteGFX::       INCBIN "gfx/sprites/gambler_asleep.2bpp"
floating_section
IF DEF(_BLUE)
PlayerBikeSpriteGFX::
ENDC
RedBikeSpriteGFX::             INCBIN "gfx/sprites/red_bike.2bpp"
floating_section
CouchManSpriteGFX::            INCBIN "gfx/sprites/couch_man.2bpp"
